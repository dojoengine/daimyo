...
entries:
  - id: "42"
    ...
    work_done_short: "Built from scratch during the jam. Full chess engine in Cairo..."
    work_done_long: "The engine handles all standard rules including castling, en passant..."
...
- **Game Summary** section: short summary (~50 words, always visible) with expandable long summary (~250 words)
- **Work Done** section: ~35 words always visible with expandable ~65 word continuation (~100 words total, flowing narrative)
- 5 metric chips (Type, Team, Contracts, Jam Commits, Playability)

Below the cards, a 5-point Likert scale:
- **Much** (1.0) / **Slightly** (0.75) / **Even** (0.5) / **Slightly** (0.25) / **Much** (0.0)
...
2. Build N×N transition matrix where N = number of entries
   ...
   - Much A (1.0): A gets full weight, B gets 0
   - Even (0.5): equal weight to both
   - Much B (0.0): B gets full weight, A gets 0
...
**EntryCard**: Title with icon links (play/video/source), Game Summary section (short + expandable long), Work Done section (short + expandable long), 5 metric chips. Section headers are clickable toggles.

**LikertScale**: 5 buttons (Much, Slightly, Even, Slightly, Much) with game titles as anchors.
