...
Judges are presented with two games at a time and choose which is better.
...
```yaml
# gj7/entries.yaml (generated by GitHub workflow)
entries:
  - id: "42"
    title: "On-Chain Chess"
    description: "A fully on-chain chess engine built with Dojo"
...
**Required Fields**: `id` (PR number), `title`, `author_github`

**Optional Fields**: `description`, `demo_url`, `video_url`, `source_url`
...
Ordinal inputs (A vs B) are preferred over cardinal (A is 3x better) for untrained audiences.
...
Judges can skip comparisons they cannot meaningfully evaluate.
This reduces bad data from indifferent or uncertain voters.
Skipped pairs are returned to the pool for other judges.

### Comparison UI

Judge sees two entry cards side-by-side (left/right position randomized per comparison):
- Game title
- Description
- "Play Game" link (opens in new tab) OR "Watch Gameplay Video"
- "This one is better" button
- "Skip" button (for incomparable pairs)
...
2. Build N×N transition matrix where N = number of entries
   - A[i][j] = number of times entry i beat entry j
...
**Score Encoding**: `score` is a real number representing preference strength:
- `1.0` = A wins (strongly)
- `0.5` = tie/equal
- `0.0` = B wins (strongly)
- `NULL` = skipped

Currently UI only supports binary choices (1.0 or 0.0), but REAL type allows future enhancement for expressing preference strength.
...
Record vote or skip.
Body: `{ entryAId, entryBId, winnerId }` where winnerId is the chosen entry (or null to skip)
Backend canonicalizes entry order and computes score (1.0 if winner is smaller ID, 0.0 otherwise, null if skip).
...
Comparisons: 2026-01-15 to 2026-01-31
 ...
 Rank  Score  PR#   Title                Author
 ----  -----  ---   -----                ------
    1  87.3    42   On-Chain Chess       @octocat
    2  82.1    57   Dojo Dungeon         @dungeondev
    3  71.5    63   Cairo Kart           @kartmaster
  ...
...
<ComparisonView>
  <EntryCard entry={entryA} onSelect={() => selectWinner(entryA.id)} />
  <div>VS</div>
  <EntryCard entry={entryB} onSelect={() => selectWinner(entryB.id)} />
</ComparisonView>
```
...
<div>
  ...
  <a href={entry.demoUrl}>Play Game</a>
  <button onClick={onSelect}>This one is better</button>
</div>
```
