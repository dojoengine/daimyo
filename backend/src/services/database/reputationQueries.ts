...
/**
 * Reputation-focused database query helpers.
 */
...
export async function getReactionBreakdown(userId: string): Promise<ReactionCountsByRole> {
  ...
  return {
    ...
    fromKohai: parseCount(row?.fromkohai),
    fromSenpai: parseCount(row?.fromsenpai),
    fromSensei: parseCount(row?.fromsensei),
  };
}
...
export async function getDetailedSenseiBreakdown(
  ...
  const row = results[0];
  return {
    reactions: parseCount(row?.reactions),
    uniqueReactors: parseCount(row?.unique_reactors),
  };
}
...
export async function getLeaderboard(role?: Role, limit: number = 20): Promise<LeaderboardEntry[]> {
  ...
  return results.map((r) => ({
    user_id: r.user_id,
    reaction_count: parseCount(r.reaction_count),
  }));
}
...
export async function getTopSenseiReactionRecipients(days: number): Promise<LeaderboardEntry[]> {
  ...
  const maxCount = parseCount(maxResult[0]?.max_count);
